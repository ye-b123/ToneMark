...
Side-by-side comparisons of Twi words where tone alone distinguishes meaning. For example, papa can mean father (pàpá), good (pápá), or fan (pàpà) depending on tone pattern so without diacritics, an NLP model sees only "papa" and must guess.
